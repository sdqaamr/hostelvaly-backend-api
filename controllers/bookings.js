...
let getBookings = async(req,res) => {
    try {
        const bookings = await Bookings.find().select(["roomType", "price", "paymentMethod", "bookingDate", "hostel", "user"]).populate(["hostel", "user"]);
        res.status(200).json({
            success: true,
...
let getBooking = async(req,res) => {
    try{
        ...
        const booking = await Bookings.findById(id).select(["roomType", "price", "paymentMethod", "bookingDate", "hostel", "user"]).populate(["hostel", "user"]);
        if(!booking) {
...
let putBooking = async(req,res) => {
    try {
        ...
        const booking = await Bookings.findByIdAndUpdate(id, req.body, { new: true }).populate(["hostel", "user"]);
        if(!booking) {
            return res.status(404).json({
            success: false,
                message: "Booking not found",
...
let deleteBooking = async(req,res) => {
    try {
        let id = req.params.id;
        const booking = await Bookings.findByIdAndDelete(id).populate(["hostel", "user"]);
        if(!booking) {
            return res.status(404).json({
                success: false,
                message: "Booking not found",
...
export { getBookings, getBooking, putBooking, postBookings, deleteBooking, deleteBookings };